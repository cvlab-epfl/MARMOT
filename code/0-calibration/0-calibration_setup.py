...
def main():
    ...
    if data_source is not None:
        ...
        if footage_path.is_symlink():
            footage_path.unlink()
        log.debug(f"Footage path not found at {footage_path}. Symlinking to data source at {data_source / 'footage'}.")
    ...
    log.info("Found calibration footage for cameras: {}".format(cam_names))

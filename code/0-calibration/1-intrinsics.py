...
default_intrinsics = config.get('calibration', {}).get('intrinsics', None)
...
def main():
    ...
    if default_intrinsics is not None:
        log.info(f"Default intrinsics being loaded from: {default_intrinsics}")
        ...
        for cam in footage_cams:
            ...
            calib = load_json(default_intrinsics)

            calib["view_id"] = camera.name.split('m')[1]

            camera.save_calibration(calib)

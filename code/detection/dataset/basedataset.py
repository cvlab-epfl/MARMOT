...
from detection.augmentation.homographyaugmentation import HomographyDataAugmentation
from detection.dataset.utils import aggregate_multi_view_gt_points, generate_scene_roi_from_view_rois, generate_mask_from_polygon_perimeter, get_augmentation
from detection.misc import geometry
from detection.misc.utils import PinnableDict, stack_tensors, listdict_to_dictlist
...
class FlowSceneSet(torch.utils.data.Dataset):
    ...
    def __init__(self, scene_set, data_conf, use_augmentation, compute_flow_stat=False):
        ...
        log.debug(f"Scene set containing {self.nb_view} view and {len(self.scene_set)} frames, will process {self.nb_frames} frame at a time")

...
from detection.model.multimodel import MultiNet
...
class MultiviewModel(torch.nn.Module):
    ...
    def forward(self, input_data, return_features=False):

        pred_0, frame_pred_0, view_feat, scene_feat   = self.model(input_data["frame_0"], input_data["homography"], input_data["ROI_mask"]) #flow_0_1f, flow_1_0b
        ...
        if return_features:
            output["view_feat"] = view_feat
            output["scene_feat"] = scene_feat

        return output
    
    def get_reid_size(self):
        return self.model.get_reid_size()
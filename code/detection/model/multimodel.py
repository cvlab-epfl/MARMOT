...
from detection.misc.geometry import project_to_ground_plane_pytorch 
...
class MultiNet(torch.nn.Module):
    def __init__(self, hm_size, homography_input_size, homography_output_size, nb_ch_out, model_image_pred=False, nb_view=3):
        ...
        self.frontend_feature_outsize = 512
        multiview_features = 128
        backend_feat = [512,256,128,64]
        self.output_layer_dim = 64
        ...
        self.frontend_feature_outsize = 128
        backend_feat = [128,128,64,32]
        self.output_layer_dim = 32
        ...
        self.multiview_layer_front = torch.nn.Conv2d(self.frontend_feature_outsize, multiview_features, kernel_size=1)
        ...
        MultiScaleBackend(multiview_features*2, backend_feat, out_features=self.output_layer_dim) #torch.nn.Conv2d(1024, 64, kernel_size=1)#None#dla34up(64)
        ])

        self.output_layer = torch.nn.Conv2d(self.output_layer_dim, nb_ch_out, kernel_size=3 ,padding=1)
    ...
    def forward(self, x, h_groundplane, roi):
        ...
        if self.image_pred:
            img_pred = self.img_classifier(fx).reshape(B,V,*fx.shape[-2:])
        else:
            img_pred = None

        #Reshape to have batchdimension before grounplane projection
        x = fx.view(B,V,fx.shape[-3],fx.shape[-2],fx.shape[-1]) 
        #project
        ...
        sx = self.multiview_layer_back(x)

        x = self.output_layer(sx)
        x = self.logit_f(x)

            
        return x, img_pred, fx, sx

    def get_reid_size(self):
        return self.frontend_feature_outsize, self.output_layer_dim

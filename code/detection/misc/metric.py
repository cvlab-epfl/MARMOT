...
from detection.dataset.utils import generate_flow
from utils.log_utils import log
from detection.misc.utils import dict_merge

def compute_detection_stats(input_data, output_data):
   
    hm_flow = output_data["pred"]["reconstruction_overall_density"].detach().unsqueeze(0)
...
def compute_flow_metrics(input_data, output_data, conf, is_flow=True):
    ...
    if is_flow:
        ...
        prev_flow_stats  = count_single_flow_stats(flow_0_1f_gt, output_data["pred"]["flow_0_1f"][0])
        post_flow_stats  = count_single_flow_stats(flow_1_2f_gt, output_data["pred"]["flow_1_2f"][0])
        prev_flow_inverse_stats  = count_single_flow_stats(flow_1_0b_gt, output_data["pred"]["flow_1_0b"][0])
        post_flow_inverse_stats  = count_single_flow_stats(flow_2_1b_gt, output_data["pred"]["flow_2_1b"][0])
...
def decode_heatmap(heatmap, kernel_size, use_nms, threshold=None, K=150):
    ...
    if threshold is not None:
        if threshold=="auto":
            kmeans = KMeans(n_clusters=2, random_state=0, n_init=10).fit(topk_scores.reshape(-1,1))
...
def compute_mot_metric(gt_df, pred_df, metric_threshold, nb_gt=None):
...
def save_detection_for_evaluation(gt, det, metric_threshold, root_path, model_name, epoch, detname):
    # import numpy as np
    res_fpath = root_path + "/detection_results/" + model_name + "/" + model_name + '_epoch_' + str(epoch)  + "_" +  detname + ".txt"
    gt_fpath = root_path + "/detection_results/" + model_name + "/" + model_name + '_epoch_' + str(epoch)  + "_" +  detname + "_gt.txt"
    os.makedirs(os.path.dirname(res_fpath), exist_ok=True)

    detections = []
    gts = []

    for i, (curr_gt, curr_det) in enumerate(zip(gt, det)):
        detections.extend([np.array([i*5, point[0], point[1]]) for point in curr_det])
        gts.extend([np.array([i*5, point[0], point[1]]) for point in curr_gt])

    if len(detections) == 0:
        log.warning("No detections found, nothing was saved")
        return 

    detections = np.stack(detections, axis=0)
    gts = np.stack(gts, axis=0)
    
    # if not os.path.exists(gt_fpath):
    np.savetxt(gt_fpath, gts, '%d')

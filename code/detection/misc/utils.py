...
def save_checkpoint(model, optimizer, lr_scheduler, conf, epoch_stats, epoch, is_best, save_all=False):
...
def check_for_existing_checkpoint(project_root, model_name, get_best=False):
    ...
    if get_best:
        file_path = project_root + "/weights/" + model_name + "/best_" + model_name + ".pth.tar"
...
def to_device(data_elements, *args, **kwargs):
    ...
    log.spam(f"Mappping {isinstance(data_elements, collections.abc.Mapping)} sequence {isinstance(data_elements, collections.abc.Sequence)} has_to {callable(getattr(data_elements, 'to', None))} {type(data_elements)}")
    ...
    if isinstance(data_elements, collections.abc.Mapping):
        return {key: to_device(value, *args, **kwargs) for key, value in data_elements.items()}
    elif isinstance(data_elements, collections.abc.Sequence) and not isinstance(data_elements, str):
...
def stack_tensors(batch):
        ...
        elif isinstance(batch, collections.abc.Sequence) and torch.is_tensor(batch[0]):

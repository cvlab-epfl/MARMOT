...
def batch_logging(epoch, batch_index, nb_batch, stats_meter, loss_to_print=[], metric_to_print=[], validation=False):
    # from detection.misc import metric
...
def batch_inf_logging(epoch, batch_index, nb_batch, stats_meter):
    # from detection.misc import metric
    ...
    if batch_index == 0:
        log.info("\t".join([str_step_lgd, str_time_lgd]))

    log.info("\t".join([str_step, str_time]))

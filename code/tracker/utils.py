...
class Track:
    ...
    def __init__(self, person_id):
        ...
        #detections.append({'FrameId':frame_id, 'Id':int(row[0]), 'X':int(float(row[2])), 'Y':int(float(row[1])), 'Confidence':1.0})
    ...
    def __str__(self):
        return f"Trackc with person_id: {self.person_id} \n" + dict_to_string(self.detections)
